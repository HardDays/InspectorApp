...
class InstructionListBloc
    extends Bloc<InstructionListBlocEvent, InstructionListBlocState> {
  ...
  @override
  Stream<InstructionListBlocState> mapEventToState(
      InstructionListBlocEvent event) async* {
    if (event is LoadEvent || event is RefreshEvent) {
      ...
      try {
        ...
        final reload = event is RefreshEvent ||
            (date == null) ||
      ...
      if (state is DataState) {
        ...
        yield DataState(
            data, state.date, state.sort, state.order, event.filters);
      }
    } else if (event is FlushEvent) {
      if (state is DataState) {
        yield DataState((state as DataState).instructions, state.date,
            state.sort, state.order, state.filters);
      }
  ...
  List<Instruction> _processData(String sort, String order,
      InstructionFilters filters, List<Instruction> data) {
    List<Instruction> result = data;
    if (filters != null) {
      if (filters.instructionStatus != null) {
        result = result
            .where((e) => e.instructionStatus.id == filters.instructionStatus)
            .toList();
      }
      if (filters.instructionNum != null) {
        result = result
            .where((e) => e.instructionNum
                .toLowerCase()
                .contains(filters.instructionNum.toLowerCase()))
            .toList();
      }
      if (filters.instructionDateFrom != null) {
        result = result
            .where(
                (e) => e.instructionDate.isAfter(filters.instructionDateFrom))
            .toList();
      }
      if (filters.instructionDateTo != null) {
        result = result
            .where((e) => e.instructionDate
                .isBefore(filters.instructionDateTo.add(Duration(days: 1))))
            .toList();
      }
      if (filters.checkDateFrom != null) {
        result = result
            .where((e) => e.checkDate.isAfter(filters.checkDateFrom))
            .toList();
      }
      if (filters.checkDateTo != null) {
        result = result
            .where((e) => e.checkDate
                .isBefore(filters.checkDateTo.add(Duration(days: 1))))
            .toList();
      }
    ...
    if (sort == InstructionSortStrings.instructionNum) {
      ...
      result.sort((c1, c2) =>
          c1.instructionStatus.id.compareTo(c2.instructionStatus.id));
    }
