...
class TotalReportPageState extends State<TotalReportPage>
  ...
  final _normativeActControllers = [TextEditingController()];
  final _normativeActArticleControllers = [TextEditingController()];
  final List<Uint8List> _photos = [];
  final List<String> _photoNames = [];
  final _violatorFormKeys = [GlobalKey<FormState>()];
  final _violatorTypeControllers = [TextEditingController()];
  final _violatorControllers = [TextEditingController()];
  final _firstNameControllers = [TextEditingController()];
  final _lastNameControllers = [TextEditingController()];
  final _patronymControllers = [TextEditingController()];
  final _docTypeControllers = [TextEditingController()];
  ...
  final _legalAddressControllers = [TextEditingController()];
  final _postalAddressControllers = [TextEditingController()];
  final _registrationAddressControllers = [TextEditingController()];
  final _birthPlaceControllers = [TextEditingController()];
  final _departmentCodeControllers = [TextEditingController()];
  final List<DateTime> _registerDates = [null];
  ...
  void initState() {
    ...
    _allViolatorControllers = [
      _violatorControllers,
      _phoneControllers,
      _firstNameControllers,
      _lastNameControllers,
      _patronymControllers,
      _docTypeControllers,
      _docSeriesControllers,
      _docNumberControllers,
      _innControllers,
      _snilsControllers,
      _ogrnControllers,
      _kppControllers,
      _legalAddressControllers,
  ...
  void _onSave(BuildContext context, int status) async {
    ...
    final res = await showDialog(
      ...
      builder: (ctx) => AcceptDialog(
          acceptTitle: 'Да',
          cancelTitle: 'Нет',
          message: status == ReportStatusIds.onApproval
              ? 'Вы подтверждаете передачу рапорта $number от ${DateFormat('dd.MM.yyyy').format(date)} на согласование?'
              : 'Сохранить рапорт?'),
    );
    if (res != null) {
      if (_photos.isEmpty) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      await showDialog(
          ...
          builder: (ctx) => AcceptDialog(
              cancelTitle: null,
              acceptTitle: 'ОК',
              message: report.reportStatus?.id == ReportStatusIds.onApproval
                  ? 'Зарегистрирован рапорт $title'
                  : 'Сохранен рапорт $title'));
  ...
  String _innValidator(int index, ViolatorDocumentType docType) {
    if (_innControllers[index].text.isEmpty &&
        !(docType != null &&
            _docNumberControllers[index].text.isNotEmpty &&
            _docSeriesControllers[index].text.isNotEmpty)) {
      return 'Введите значение';
    }
    return null;
  }
        ...
        child: BlocBuilder<TotalReportBloc, TotalReportBlocState>(
          ...
          return Scaffold(
            ...
            appBar: ProjectAppbar(
              'Рапорт ${widget.report.reportNum ?? ''}',
            ),
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    state.report.violationNotPresent
                        ? _buildViolationNotPresent(context, state)
                        : _buildViolationPresent(context, state),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildError(BuildContext context, Report report) {
    if (report.error != null && report.error.isNotEmpty) {
      return Container(
        ...
        child: Text(
          'Рапорт № ${report.reportNum} от ${DateFormat('dd.MM.yyyy').format(report.reportDate)} не был загружен в ЕИС ОАТИ. Ошибка: ${report.error}',
  ...
  Widget _buildViolator(BuildContext context, Violator violator, int index,
    ...
    return Theme(
      ...
      child: ExpansionTile(
        ...
        title: Text('Нарушитель ${index + 1}', style: ProjectTextStyles.base),
        children: [
          Padding(
  ...
  Widget _buildViolatorInfo(BuildContext context, int index, Violator violator,
    ...
    return Column(
      children: [
        ...
        if (resolution != null) ResolutionWidget(resolution: resolution),
      ],
    );
  }
