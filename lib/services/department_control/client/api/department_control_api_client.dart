...
class DepartmentControlApiClient with DepartmentControlServiceClient {
  ...
  Future<List<ControlObject>> getControlObjects(
          DepartmentControlObjectsRequest request) async =>
      apiProvider
          .getControlObjects(
          ...
          .then((response) => response['data']
              .map((e) => ControlObject.fromJson(e))
              .cast<ControlObject>()
              .toList());

  Future<List<ControlResultSearchResult>> getControlSearchResults(
          DepartmentControlSearchResultsRequest request) async =>
      apiProvider
          .getControlSearchResults(
            request.dcObjectId,
            forCurrentUser: request.forCurrentUser,
  ...
  Future<ControlResult> registerControlResult(
    ...
    return ControlResult.fromJson(await apiProvider.createDCControlResult(
        request.object.id, request.controlResult));
  }

  Future<ControlResult> updateControlResult(
      DepartmentControlUpdateControlRequest request) async {
    return ControlResult.fromJson(await apiProvider.updateDCControlResult(
        request.object.id, request.dcControlResultId, request.violation));
  }
  ...
  Future<PerformControl> registerPerformControl(
          DepartmentControlRegisterPerformControlRequest request) async =>
      PerformControl.fromJson(await apiProvider.createDCPerformControlResult(
          request.object.id,
          request.dcControlResultId,
          request.performControl));

  Future<PerformControl> updatePerformControl(
          DepartmentControlUpdatePerformControlRequest request) async =>
      PerformControl.fromJson(await apiProvider.updateDCPerformControlResult(
          request.object.id,
          request.dcControlResultId,
          request.performControl));

  Future<void> removePerformControl(
          DepartmentControlRemovePerformControlRequest request) async =>
      await apiProvider.removeDCPerformControlResult(request.object.id,
          request.dcControlResultId, request.performControl.id);

  Future<void> extendPeriod(
      DepartmentControlExtendControlPeriodRequest request) async {
    await apiProvider.extendPeriod(request.object.id, request.dcControlResultId,
